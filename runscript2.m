function [status,location] = runscript2(fig,from,to,speed)
...
kinematicsConfig.cruiseSpeed = speed; % m/s
...
if from == "SAF"
    a = 1;
elseif from == "Southside"
    a = 2;
elseif from == "Prince's Garden"
    a = 3;
elseif from == "CAGB"
    a = 4;
elseif from == "Skempton"
    a = 5;
elseif from == "Queen's Lawn"
    a = 6;
elseif from == "Library"
    a = 7;
elseif from == "Sherfield"
    a = 8;
elseif from == "Huxley"
    a = 9;
elseif from == "Blackett"
    a = 10;
elseif from == "Roderic Hill"
    a = 11;
elseif from == "RSM"
    a = 12;
elseif from == "Sports Centre"
    a = 13;
elseif from == "Beit Quad"
    a = 14;
end
...
elseif to == "Southside"
    b = 2;
elseif to == "Prince's Garden"
    b = 3;
elseif to == "CAGB"
    b = 4;
elseif to == "Skempton"
    b = 5;
elseif to == "Queen's Lawn"
    b = 6;
elseif to == "Library"
    b = 7;
elseif to == "Sherfield"
...
elseif to == "Blackett"
    b = 10;
elseif to == "Roderic Hill"
    b = 11;
elseif to == "RSM"
    b = 12;
elseif to == "Sports Centre"
    b = 13;
elseif to == "Beit Quad"
    b = 14;
end
...
if a<b
    if manualPath(a+1,1)==manualPath(a,1)
        if manualPath(a+1,2)>manualPath(a,2)
            rad = pi/2;
        elseif manualPath(a+1,2)<manualPath(a,2)
            rad = -pi/2;
        end
    elseif manualPath(a+1,2)==manualPath(a,2)
        if manualPath(a+1,1)>manualPath(a,1)
            rad = 0;
        elseif manualPath(a+1,1)<manualPath(a,1)
            rad = pi;
        end
    else
        rad = atan((manualPath(a+1,2)-manualPath(a,2))/(manualPath(a+1,1)-manualPath(a,1)));
        if manualPath(a+1,1)<manualPath(a,1) 
            if rad < 0
                rad = rad + pi;
            else
    ...
    if manualPath(b,1)==manualPath(b-1,1)
        if manualPath(b,2)>manualPath(b-1,2)
            rad2 = pi/2;
        elseif manualPath(b,2)<manualPath(b-1,2)
            rad2 = -pi/2;
        end
    elseif manualPath(b,2)==manualPath(b-1,2)
        if manualPath(b,1)>manualPath(b-1,1)
            rad2 = 0;
        elseif manualPath(b,1)<manualPath(b-1,1)
            rad2 = pi;
        end
    else
        rad2 = atan((manualPath(b,2)-manualPath(b-1,2))/(manualPath(b,1)-manualPath(b-1,1)));
        if manualPath(b,1)<manualPath(b-1,1)
            if rad2 < 0
                rad2 = rad2 + pi;
            else
...
elseif a>b
    ...
    msg = "Airship is already at destination!";
    status = msg;
    rad = 0;
    rad2 = 0;
end
disp(rad);
disp(rad2);
initPose = [manualPath(a,1), manualPath(a,2), rad];
poses = zeros(3,numel(t));    % Pose matrix
estposes = zeros(3,numel(t));    % Estimated Pose matrix
poses(:,1) = initPose';
goalPoint = [manualPath(b,1) manualPath(b,2)]';
goalPose = [manualPath(b,1) manualPath(b,2) rad2];
...
for idx = 1:numel(t)
    ...
    if (dist < 1)
        msg = "Destination has been reached!";
        status = msg;
        location = to;
        break;
    end
...
%             pthObj = plan(planner, position, goalPose);
    ...
    % Update position
    %newspeed = updatespeed(update);
    %kinematicsConfig.cruiseSpeed = newspeed;
